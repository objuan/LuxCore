...
public:
    ...
    static void Init(int initialSize) {
        memory = malloc(initialSize);
        maxSize = initialSize;
        size = 0;
...
namespace RealBloom
{
    ...
    class MyConvolutionThread
    {
        ...
        bool _isEnd;
    public:
        MyConvolutionThread(int index, ConvolutionFFT* conv) : fftConv(conv), index(index), _isEnd(false)
        {
        }
        ~MyConvolutionThread() {
            if (thread != NULL)
