					...
					if (!scene->Intersect(device, EYE_RAY | SHADOW_RAY, &volInfo, u4, &shadowRay,
						...
						if (!bsdf.IsShadowCatcher()) {
							...
							if (pathGuiding->enabled)
								pathGuiding->guiding_record_surface_emission(pathThroughput, incomingRadiance, 1.f);
						}
...
void PathTracer::DirectHitFiniteLight(const Scene *scene,
	...
	if (!emittedRadiance.Black()) {
		...
		if (pathGuiding->enabled)
			pathGuiding->guiding_record_surface_emission(pathThroughput,emittedRadiance, weight);
...
void PathTracer::DirectHitInfiniteLight(const Scene *scene,
	...
	BOOST_FOREACH(EnvLightSource *envLight, scene->lightDefs.GetEnvLightSources()) {
		...
		if (!envRadiance.Black()) {
			...
			if (pathGuiding->enabled)
				pathGuiding->guiding_record_surface_emission(pathThroughput,envRadiance, weight);
...
void PathTracer::RenderEyePath(IntersectionDevice *device,
	...
	//pathGuiding->guiding_record_surface_segment(eyeRay.o, eyeRay.d);
	//
	if (log_file != NULL)
		(*log_file) << "===============RAY "<< log("dir", eyeRay.d) << "\n";
	...
	for (;;) {
		...
		const bool hit = scene->Intersect(device,
			...
			// OUT: bsdf , connectionThroughput
		// se vcolume sampleResult->AddEmission
		...
		if (!hit) {
			...
			if (pathGuiding->enabled)
				pathGuiding->guiding_record_background(sampleResult.albedo, 1);
		...
		if (pathGuiding->enabled)
			pathGuiding->guiding_record_surface_segment(bsdf.hitPoint.p, -Normalize(eyeRay.d));


		if (pathGuiding->enabled)
			pathGuiding->surface_shader_prepare_guiding(sampleResult.position, sampleResult.geometryNormal);
		...
		//	bsdfSample = ShadowCatcherSample(&bsdf ,&sampledDir, &bsdfPdfW, &cosSampledDir, &bsdfEvent);
		...
		//else 
		{
			...
			if (!sampleResult.firstPathVertex && !shadowTransparency.Black() && !pathInfo.IsSpecularPath()) {
				...
				bool toContinue = SampleBSDF(&bsdf,&sampledDir,
		...
		if (pathGuiding->enabled)
		{
			...
			Normal N = bsdf.hitPoint.geometryN;
			float3 d = float3(sampledDir.x, sampledDir.y, sampledDir.z);
			pathGuiding->guiding_record_surface_bounce(bsdfSample, bsdfPdfW,
...
void PathTracer::ParseOptions(const luxrays::Properties &cfg, const luxrays::Properties &defaultProps) {
	...
	eyeSampleStepSize = 9+2; // per guiding
	eyeSampleGuiding = 9;
...
Spectrum PathTracer::ShadowCatcherSample(BSDF* bsdf,Vector* sampledDir,
...
//Spectrum PathTracer::SampleBSDF(BSDF* bsdf, Vector* sampledDir,
...
//	// ===================================
//	// 
//	//GUIDING , work in world coordinate
//	//float guiding_sampling_prob=1.0f;
//
//	float rand = u0;
//	if (pathGuiding->state->use_surface_guiding)
//		//if (pathGuiding->guiding_bsdf_init(hitPoint.p, hitPoint.geometryN, rand))
//		{
//			float guiding_sampling_prob = pathGuiding->state->surface_guiding_sampling_prob;
//
//			float3 wo;
//			float guide_pdf = pathGuiding->guiding_bsdf_sample(float2(u0, u1), &wo);
//		
//			Vector _localSampledDir = Vector(wo.x, wo.y, wo.z);
//			//_localSampledDir = Vector(1, 0, 0);
//			_localSampledDir = bsdf->GetFrame().ToLocal(_localSampledDir);
//
//			//*pdfW = *pdfW * (1.0f - guiding_sampling_prob)	+ guide_pdf * guiding_sampling_prob;
//			localSampledDir = localSampledDir * (1.0f - guiding_sampling_prob) + _localSampledDir * guiding_sampling_prob;
//			localSampledDir = Normalize(localSampledDir);
//	
//	
//			//float directPdfW=0;
//			//float reversePdfW = 0;
//			//result = Spectrum(1, 1, 1);// bsdf->GetMaterial()->Evaluate(hitPoint, localSampledDir, localFixedDir, event, &directPdfW, &reversePdfW);
//
//			float direct, reverse;
//			//Spectrum result1 = bsdf->Evaluate(localSampledDir, event, &direct, &reverse);
//
//			Spectrum result1 = bsdf->GetMaterial()->Evaluate(hitPoint, localSampledDir, localFixedDir, event, &direct, &reverse);
//			if (direct!=0)
//				result1 = result1 / direct;
//			// 
//			(*pdfW) = direct;
//			result = result1;
//
...
//
//		float directPDF;
//		Spectrum res1 = bsdf->GetMaterial()->Evaluate(hitPoint, localLightDir, localEyeDir, event,&directPDF);
//		
//		res1 = res1 / directPDF;
//		int u = 0;
//
...
//
//	// bsdf_sample
//	Spectrum result1 = bsdf->GetMaterial()->Sample(hitPoint,
//		localFixedDir, &localSampledDir, u0, u1, hitPoint.passThroughEvent,
//		pdfW, event);
//
//	/*const Vector& localLightDir = hitPoint.fromLight ? localFixedDir : localSampledDir;
...
//	*sampledDir = bsdf->GetFrame().ToWorld(localSampledDir);
//
//	*pdfW = *bsdf_pdf;// ris_samples[ris_idx].bsdf_pdf;
//	*out_result = ris_samples[ris_idx].bsdf_eval.sum;
//	//return true;
//
//	// 
//	//label = ris_samples[ris_idx].label;
