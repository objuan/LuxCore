...
Film::FilmChannels BiDirCPUBGLRenderThread::eyeSampleResultsChannels;

//const Film::FilmChannels BiDirCPUBGLRenderThread::eyeSampleResultsChannels({
...
Film::FilmChannels BiDirCPUBGLRenderThread::lightSampleResultsChannels;/* ({
	Film::RADIANCE_PER_SCREEN_NORMALIZED
}); */

BiDirCPUBGLRenderThread::BiDirCPUBGLRenderThread(BiDirCPUBGLRenderEngine *engine,
...
void BiDirCPUBGLRenderThread::AOVWarmUp(RandomGenerator *rndGen) {
	if (threadIndex == 0)
		SLG_LOG("[BiDirCPUBGLRenderThread::" << threadIndex << "] AOV warmup started");
	
	const double start = WallClockTime();
	double lastProgressPrint = start;
		
	BiDirCPUBGLRenderEngine *engine = (BiDirCPUBGLRenderEngine *)renderEngine;
	...
	sampleResultsChannels.insert(Film::ALBEDO);
	sampleResultsChannels.insert(Film::AVG_SHADING_NORMAL);
	...
	while (sampler.GetPassCount() < engine->aovWarmupSPP) {
		...
		if (threadIndex == 0) {
			...
			if (delta > 2.0) {
				...
				SLG_LOG("[BiDirCPUBGLRenderThread::" << threadIndex << "] AOV warmup progress: " <<
	...
	if (threadIndex == 0) {
		...
		SLG_LOG("[BiDirCPUBGLRenderThread::" << threadIndex << "] AOV warmup done: " <<
				boost::str(boost::format("%3.2fM") % samplesSec) << " samples/sec");
	}
}

SampleResult &BiDirCPUBGLRenderThread::AddResult(vector<SampleResult> &sampleResults, const bool fromLight) const {
	BiDirCPUBGLRenderEngine *engine = (BiDirCPUBGLRenderEngine *)renderEngine;
...
void BiDirCPUBGLRenderThread::ConnectVertices(const float time,
...
void BiDirCPUBGLRenderThread::ConnectToEye(const float time,
...
void BiDirCPUBGLRenderThread::DirectLightSampling(const float time,
...
void BiDirCPUBGLRenderThread::DirectHitLight(
...
void BiDirCPUBGLRenderThread::DirectHitLight(const bool finiteLightSource,
...
bool BiDirCPUBGLRenderThread::TraceLightPath(const float time,
...
bool BiDirCPUBGLRenderThread::Bounce(const float time, Sampler *sampler,
...
void BiDirCPUBGLRenderThread::RenderFunc() {
	//SLG_LOG("[BiDirCPUBGLRenderThread::" << threadIndex << "] Rendering thread started");
...
//	kg.opgl_volume_sampling_distribution = new openpgl::cpp::VolumeSamplingDistribution(guiding_field);
	...
	// sampleBootSize = 5
	// // maxLightPathDepth=5 , sampleLightStepSize=5
	// maxEyePathDepth=5 sampleEyeStepSize=10
	// Setup the sampler
	...
	for(u_int steps = 0; !boost::this_thread::interruption_requested(); ++steps) {
		...
		// OUT lightPathVertices  light path
		// sampleResults -> connectedToEye samples
	...
	//SLG_LOG("[BiDirCPUBGLRenderThread::" << threadIndex << "] Rendering thread halted");
}
