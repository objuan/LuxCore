...
MetropolisFastSampler::MetropolisFastSampler(RandomGenerator *rnd, Film *flm,
	...
	MetropolisSamplerSharedData*samplerSharedData) : MetropolisSampler(rnd, flm, flmSplatter, imgSamplesEnable,
			maxRej, pLarge, imgRange, addOnlyCstcs, samplerSharedData)
 {
}

MetropolisFastSampler::~MetropolisFastSampler() {

}


Properties MetropolisFastSampler::ToProperties(const Properties& cfg) {
	return Properties() <<
...
Sampler* MetropolisFastSampler::FromProperties(const Properties& cfg, RandomGenerator* rndGen,
	...
	return new MetropolisFastSampler(rndGen, film, flmSplatter, imageSamplesEnable,
		reject, rate, mutationRate, addOnlyCaustics,
		(MetropolisSamplerSharedData*)sharedData);
}
void MetropolisFastSampler::AddRequiredChannels(Film::FilmChannels& channels, const luxrays::Properties& cfg) {
	// No additional channels required
}

const Properties& MetropolisFastSampler::GetDefaultProps() {
...
void MetropolisFastSampler::AtomicAddSampleToFilm(const u_int threadIndex, const SampleResult& sampleResult, const float weight ) 
{
	if (sampleResult.useFilmSplat && filmSplatter)
	{
		const u_int* subRegion = film->GetSubRegion();
		const Filter* filter = filmSplatter->GetFilter();
		...
		if (!filter || (filter->GetType() == FILTER_NONE)) {
			...
			if ((x >= (int)subRegion[0]) && (x <= (int)subRegion[1]) && (y >= (int)subRegion[2]) && (y <= (int)subRegion[3])) {
				((FastFilm*)film)->AddThreadSample( this->threadIndex, x, y, sampleResult, weight);
			}
		}
		else {
			//----------------------------------------------------------------------
			...
			if (film->HasDataChannel()) {
				...
				if ((x >= (int)subRegion[0]) && (x <= (int)subRegion[1]) && (y >= (int)subRegion[2]) && (y <= (int)subRegion[3]))
					((FastFilm*)film)->AddThreadSampleResultData(threadIndex,x, y, sampleResult);
			}

			//----------------------------------------------------------------------
			// Add all color related information (filtered)
			//----------------------------------------------------------------------
			const FilterLUTs* filterLUTs = filmSplatter->GetFilterLUTs();
			...
			for (int iy = y0; iy < y1; ++iy) {
				...
				for (int ix = x0; ix < x1; ++ix) {
					...
					((FastFilm*)film)->AddThreadSampleResultColor(threadIndex,ix, iy, sampleResult, filteredWeight);
				}
			}
		}
	}
	//	filmSplatter->AtomicSplatSample(*film, sampleResult, weight);
		...
		((FastFilm*)film)->AddThreadSample(threadIndex,sampleResult.pixelX, sampleResult.pixelY, sampleResult, weight);
}


void MetropolisFastSampler::NextSample(const vector<SampleResult>& sampleResults) {
	//--------------------------------------------------------------------------
	...
	if (film) {
		...
		((FastFilm*)film)->StartFrame(threadIndex);
	...
	//SLG_LOG("[MetropolisFastSampler::" << sampleResults.size() << "] NextSample ");
	...
	if ((accProb == 1.f) || (rndVal < accProb)) {
			...
			if (film) {
				for (auto const& sr : currentSampleResults) {
					if (!addOnlyCuastics || (sr.HasChannel(Film::RADIANCE_PER_SCREEN_NORMALIZED) && sr.isCaustic))
						AtomicAddSampleToFilm(threadIndex,sr, norm);
				}
			}
		}
		...
		if (film) 	((FastFilm*)film)->EndFrame(threadIndex);
